...
function findFirstSingleChar(str) {
  return str
    .split('')
    .find((ch, _, arr) => arr.filter((c) => c === ch).length === 1) || null;
}
...
function isBracketsBalanced(str) {
  ...
  const removePairs = (s) => {
    const next = pairs.reduce((acc, p) => acc.split(p).join(''), s);
    return next === s ? next : removePairs(next);
  };
...
function getMatrixProduct(m1, m2) {
  ...
  return [...Array(rows).keys()].map((i) => [...Array(cols)
    .keys()].map((j) => [...m1[i].keys()]
    .reduce((sum, k) => sum + m1[i][k] * m2[k][j], 0)));
}
...
function evaluateTicTacToePosition(position) {
  ...
  return lines
    .map((line) => {
      ...
      return v1 && v1 === v2 && v1 === v3 ? v1 : undefined;
    })
    .find((v) => v !== undefined);
}
